...
{% block content %}
    ...
    <div class="account-orders section">
        <div class="{{ layout_container }}">
            <h2 class="account-orders-title title title_mb-lg">{{ 'lang.storefront.account.my_account'|t }}</h2>
                ...
                <div class="col-lg-9">
                    ...
                    {% if user.order_detail %}
                        ...
                        <div class="order-action-buttons">
                            ...
                            {% if user.order_detail.invoice_url %}<a href="{{ user.order_detail.invoice_url }}" target="_blank" class="btn btn-default {{ store.theme_options.button_default_shadow }} btn-sm">{{ icons('file-text') }} {{ 'lang.storefront.order.invoice'|t }}</a>{% endif %}
                            {% if not user.order_detail.return_request and store.settings.order_return_active and user.order_detail.status == 8 and user.order_detail.delivered_at %}<button type="button" data-toggle="modal" data-target="#return-modal" class="btn btn-default btn-sm" target="_blank">{{ icons('corner-up-left') }} {{ 'lang.storefront.account.orders.order_detail.return.button'|t }}</button> &nbsp; {% endif %}
                        </div>
                        ...
                        {% if user.order_detail.return_request %}
                            <div class="well well-default {{ store.theme_options.well_default_shadow }}">
                                            ...
                                            {% if return_custom_field.internal.products is defined and return_custom_field.internal.products|length > 0 %}
                                                {% for cf_product in return_custom_field.internal.products %}
                                                    {% if cf_product.title is defined and cf_product.quantity is defined %}
                                                        <li><span class="badge badge-outline">{{ cf_product.quantity }}x</span> {{ cf_product.title }}</li>
                                                    {% else %}
                                                        {% for sub_product in cf_product %}
                                                            {% if sub_product.title is defined and sub_product.quantity is defined %}
                                                                <li><span class="badge badge-outline">{{ sub_product.quantity }}x</span> {{ sub_product.title }}</li>
                                                            {% endif %}
                                                        {% endfor %}
                                                    {% endif %}
                                                {% endfor %}
                                ...
                                {% if user.order_detail.return_url %}
                                    ...
                                    <a href="{{ site_url('download-return-file/' ~ user.order_detail.hash) }}" target="_blank" class="btn btn-default btn-sm margin-top-xs">{{ icons('file-text') }} {{ 'lang.storefront.account.orders.order_detail.return.download_guide'|t }}</a>
                                {% else %}
                                    {% if user.order_detail.status != '9' %}
                                        <hr>
                                        <a href="{{ site_url('cancel-return-request/' ~ user.order_detail.hash) }}" class="btn btn-default btn-sm">{{ icons('trash') }} {{ 'lang.storefront.account.orders.order_detail.return.remove_return'|t }}</a>
                                    {% endif %}
                                {% endif %}
                            </div>
                        {% endif %}
                        ...
                        <div class="row margin-bottom">
                            <div class="col-sm-6">
                                ...
                                {% if not user.order_detail.paid and user.order_detail.status_alias != 'canceled' %}
                                    <h5 class="margin-bottom-xxs margin-top-visible-sm bold">{{ 'lang.storefront.order.payment_data'|t }}</h5>
                                    <div class="row">
                                        <div class="col-sm-9">
                            ...
                            <div class="col-sm-6">
                                ...
                                {% else %}
                                    <h4 class="margin-bottom-xxs margin-top-visible-sm">{{ 'lang.storefront.order.shipment'|t }}</h4>
                        ...
                        <div class="row margin-bottom">
                            ...
                            <div class="d-block d-md-none margin-bottom"></div>
                            <div class="col-sm-6">
                                <h4 class="margin-bottom-xxs margin-top-visible-sm">{{ 'lang.storefront.order.billing.address'|t }}</h4>
                                <p>
                        ...
                        {% if user.order_detail.products %}
                            <div class="order-cart">
                                <div class="order-cart-row">
                                    <div class="order-cart-col">
                                        <div class="order-cart-list">
                                            {% for product in user.order_detail.products|filter(product => product.is_product) %}
                                                <div class="order-cart-item well-featured {{ store.theme_options.well_featured_shadow }}">
                                                    <a class="item-preview" href="{{ product.url }}">
                                                        <img class="item-pic lazy" src="{{ assets_url('assets/store/img/no-img.png') }}" data-src="{{ product.image.square }}" alt="{{ product.image.alt ? product.image.alt : product.title|e }}" title="{{ product.title|e }}">
                                                    </a>
                                                    <div class="item-details">
                                                        <a class="item-product link-inherit" href="{{ product.url }}">{{ product.title }}</a>
                                                                ...
                                                                <ul class="list-group extra-options collapse in margin-bottom-0 margin-top-xs" id="item-extra-{{ product.id ~ '-' ~ extras_slug }}">
                                                                    {% for key, extra in product.extras %}
                                                                        <li class="list-group-item">
                                                                            ...
                                                                            <div class="text-truncate small margin-top-xxs" style="max-width: 200px; min-width: 100%" data-toggle="tooltip" title="{{ extra.value }}"><span class="text-muted">{{ extra.quantity }}x</span> {{ extra.value }}</div>
                                                                        </li>
                                                                    {% endfor %}
                                                                </ul>
                                                            </div>
                                                        ...
                                                        {% if product.files %}
                                                            ...
                                                            <div class="digital-files-wrapper">
                                                                <ul class="list-group well-featured {{ store.theme_options.well_featured_shadow }} digital-files collapse show in margin-bottom-xs margin-top-xs">
                                    ...
                                    <div class="order-cart-col">
                                        <div class="order-cart-receipt well-featured {{ store.theme_options.well_featured_shadow }}">
                                            <div class="order-cart-category">{{ 'lang.storefront.cart.order_summary.title'|t }}</div>
                                            <div class="order-cart-wrap">
                                                <div class="order-cart-line">
                                                    <div class="order-cart-text">{{ 'lang.storefront.order.shipping.title'|t }}:</div>
                                                    <div class="order-cart-text">{{ user.order_detail.shipping.value|money_with_sign(user.order_detail.currency) }}</div>
                                                </div>

                                                {% if user.order_detail.total_tax > 0 %}
                                                    <div class="order-cart-line">
                                                        <div class="order-cart-text">{{ user.order_detail.l10n.tax_name }}:</div>
                                                        <div class="order-cart-text">{{ user.order_detail.total_tax|money_with_sign(user.order_detail.currency) }}</div>
                                                    </div>
                                                {% endif %}
                                                ...
                                                <div class="order-cart-line">
                                                    <div class="order-cart-text">{{ 'lang.storefront.order.total'|t }}:</div>
                                                    <div class="order-cart-text">{{ user.order_detail.total|money_with_sign(user.order_detail.currency) }}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    {% else %}
                        ...
                        <h3 class="margin-bottom-xs margin-top-0 text-gray light">{{ 'lang.storefront.layout.orders.title'|t }}</h3>
