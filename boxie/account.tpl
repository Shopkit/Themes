...
{% macro account_navigation() %}
    <div class="list-group account-menu well-featured {{ store.theme_options.well_featured_shadow }}">
        ...
        {% if store.settings.rewards.active %}<a href="{{ site_url('account/rewards')}}" class="list-group-item {{ current_page == 'account-rewards' ? 'active' }} link-inherit">{{ icons('trophy', 'fa-fw fa-lg') }} {{ store.settings.rewards.plural_label ?: 'lang.storefront.account.rewards.plural.label'|t }}</a>{% endif %}
