		...
		<section class="page">
			...
			{% if cart.items %}
				...
				{{ form_open('cart/complete', { 'class' : 'form' }) }}
					...
					<table class="table table-bordered table-cart margin-bottom-md well-featured {{ store.theme_options.well_featured_shadow }} hidden-phone">
						<thead>
							<tr>
								...
								<div class="cart-item-header">
									...
									<div class="cart-item-details">
										<div class="cart-item-title">
											<a href="{{ item.product_url }}" class="semi-bold">
												<h6>{{ item.title }}</h6>
											</a>
											<div class="unit-price text-muted small">{{ 'lang.storefront.cart.product.unit_price.label'|t }}
												<span class="semi-bold">{{ item.price | money_with_sign }}</span>
											</div>
										</div>
									</div>
								</div>
								...
								<div class="cart-item-controls">
									{% if item.extras %}
										<div class="cart-item-extras well-default {{ store.theme_options.well_default_shadow }}">
											<a href="#mobile-item-extra-{{ item.item_id }}" class="inline-block small" data-toggle="collapse">{{ item.extras|length }}
												{{ item.extras|length > 1 ? 'lang.storefront.product.extra_options.plural.label'|t : 'lang.storefront.product.extra_options.singular.label'|t }}
												<span class="text-muted">({{ item.subtotal_extras > 0 ? item.subtotal_extras | money_with_sign : 'lang.storefront.cart.order_summary.shipping_total.free'|t }})</span>
												{{ icons('angle-down') }}</a>
												...
												{% for key, extra in item.extras %}
													<li class="list-group-item">
														<div class="list-group-item-header">
															<h6 class="margin-0 semi-bold">{{ extra.title }}</h6>
															<span class="badge badge-transparent normal">
																<span class="text-muted">{{ extra.qty }}x</span>
																{{ extra.price ?  extra.price | money_with_sign : 'lang.storefront.cart.order_summary.shipping_total.free'|t }}</span>
														</div>
					...
					<div class="row-fluid word-break">
						<div class="span6">
