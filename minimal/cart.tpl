			...
			{{ form_open('cart/post/data', {'class' : 'form'}) }}
				<div class="row">
					<div class="col-md-8 col-lg-8">
						...
						<div class="table-cart-products well-featured {{ store.theme_options.well_featured_shadow }} margin-bottom hidden-xs">
									...
									<tbody>
										...
										{% for item in cart.items %}
											<tr data-product="{{ item.product_id }}" data-product-option="{{ item.options|keys[0] }}">
												<td class="cart-img">
													<a href="{{ item.product_url }}"><img src="{{ assets_url('assets/store/img/no-img.png') }}" data-src="{{ item.image }}" alt="{{ item.image.alt ? item.image.alt : item.title|e_attr }}" title="{{ item.title|e_attr }}" class="border-radius lazy"></a>
												</td>
												...
												<td class="text-right">
													...
													<a href="{{ item.remove_link }}" class="btn btn-link btn-sm text-default inline-block margin-left-xxs cart-remove-product-url">{{ icons('trash') }}</a>
												</td>
											</tr>
										{% endfor %}
							...
							<footer>
								<button type="submit" formaction="{{ site_url('cart/post/update') }}" class="btn btn-default {{ store.theme_options.button_default_shadow }}">{{ icons('sync') }} {{ 'lang.storefront.cart.update.button'|t }}</button> &nbsp;
								<button type="submit" formaction="{{ site_url('cart/post/data') }}" class="btn btn-primary {{ store.theme_options.button_primary_shadow }}">{{ icons('shopping-cart') }} {{ 'lang.storefront.layout.button.buy'|t }}</button>
							</footer>
						</div>

						<div class="mobile-cart well-featured {{ store.theme_options.well_featured_shadow }} margin-bottom visible-xs">
                            {% for item in cart.items %}
